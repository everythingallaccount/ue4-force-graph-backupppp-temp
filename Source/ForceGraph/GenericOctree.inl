	...
	const FBoxCenterAndExtent& QueryBounds) const
{
	...
	Result.bPositiveX = VectorAnyGreaterThan(VectorReplicate(QueryBoundsMax,0),
	                                         VectorReplicate(PositiveChildBoundsMin,0)) != false;
	Result.bPositiveY = VectorAnyGreaterThan(VectorReplicate(QueryBoundsMax,1),
	                                         VectorReplicate(PositiveChildBoundsMin,1)) != false;
	Result.bPositiveZ = VectorAnyGreaterThan(VectorReplicate(QueryBoundsMax,2),
	                                         VectorReplicate(PositiveChildBoundsMin,2)) != false;
	Result.bNegativeX = VectorAnyGreaterThan(VectorReplicate(QueryBoundsMin,0),
	                                         VectorReplicate(NegativeChildBoundsMax,0)) == false;
	Result.bNegativeY = VectorAnyGreaterThan(VectorReplicate(QueryBoundsMin,1),
	                                         VectorReplicate(NegativeChildBoundsMax,1)) == false;
	Result.bNegativeZ = VectorAnyGreaterThan(VectorReplicate(QueryBoundsMin,2),
	                                         VectorReplicate(NegativeChildBoundsMax,2)) == false;
	return Result;
}
...
FORCEINLINE FOctreeChildNodeRef FOctreeNodeContext::GetContainingChild(const FBoxCenterAndExtent& QueryBounds) const
{
	...
	const VectorRegister NegativeCenterDifference = VectorSubtract(QueryBoundsCenter,
	                                                               VectorSubtract(
		                                                               BoundsCenter, ChildCenterOffsetVector));
	const VectorRegister PositiveCenterDifference = VectorSubtract(VectorAdd(BoundsCenter, ChildCenterOffsetVector),
	                                                               QueryBoundsCenter);
...
)
{
	...
	)
	{
		...
		if (bIsLeaf)
		{
			...
			if (Node.Elements.Num() + 1 >
				OctreeSemantics::MaxElementsPerLeaf
				&&
				Context.Bounds.Extent.X >
				MinLeafExtent
			)
			{
...
void TOctree<ElementType, OctreeSemantics>::RemoveElement(FOctreeElementId ElementId)
{
	...
	if (CollapseNode && !CollapseNode->bIsLeaf)
	{
		if (CollapseNode->Elements.Num() < (int32)CollapseNode->InclusiveNumElements)
		{
			...
			for (TConstIterator<> ChildNodeIt(*CollapseNode, RootNodeContext); ChildNodeIt.HasPendingNodes();
			     ChildNodeIt.Advance())
			{
