...
public:
	...
	friend FORCEINLINE bool Intersect(const FBoxCenterAndExtent3& A, const FBoxCenterAndExtent3& B)
	{
		...
		const VectorRegister CenterDifference = VectorAbs(
			VectorSubtract(VectorLoadAligned(&A.Center),VectorLoadAligned(&B.Center)));
	...
	friend FORCEINLINE bool Intersect(const FBoxSphereBounds& A, const FBoxCenterAndExtent3& B)
	{
		// CenterDifference is the vector between the centers of the bounding boxes.
		const VectorRegister CenterDifference = VectorAbs(
			VectorSubtract(VectorLoadFloat3_W0(&A.Origin),VectorLoadAligned(&B.Center)));

		// CompositeExtent is the extent of the bounding box which is the convolution of A with B.
		const VectorRegister CompositeExtent =
			VectorAdd(VectorLoadFloat3_W0(&A.BoxExtent),VectorLoadAligned(&B.Extent));
	...
	friend FORCEINLINE bool Intersect(const float A[4], const FBoxCenterAndExtent3& B)
	{
		// CenterDifference is the vector between the centers of the bounding boxes.
		const VectorRegister CenterDifference = VectorAbs(
			VectorSubtract(VectorLoadFloat3_W0(A),VectorLoadAligned(&B.Center)));

		// CompositeExtent is the extent of the bounding box which is the convolution of A with B.
		const VectorRegister CompositeExtent = VectorAdd(
			VectorSet_W0(VectorLoadFloat1(A+3)),VectorLoadAligned(&B.Extent));
...
public:
	...
	FORCEINLINE void GetChildContext(FOctreeChildNodeRef3 ChildRef, FOctreeNodeContext3* RESTRICT ChildContext) const
	{
		...
		ChildContext->Bounds.Center.Y = ParentContext->Bounds.Center.Y + ParentContext->ChildCenterOffset *
			NegativeOneOneTable[ChildRef.Y];
		ChildContext->Bounds.Center.Z = ParentContext->Bounds.Center.Z + ParentContext->ChildCenterOffset *
			NegativeOneOneTable[ChildRef.Z];
